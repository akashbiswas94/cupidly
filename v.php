...
$masked_ip = (count($ip_parts) === 4) ? $ip_parts[0].'.'.$ip_parts[1].'.'.$ip_parts[2].'.XXX' : "Hidden IP";
...
if ($location_json) {
    ...
    if ($loc_data && $loc_data['status']==='success') $location_text = $loc_data['city'] . ", " . $loc_data['country'];
}
...
// Update viewed_at for first-time view
if (!$p["viewed_at"]) $conn->query("UPDATE proposals SET viewed_at=NOW() WHERE id={$p['id']}");
...
#dance-container{position:absolute;inset:0;pointer-events:none;z-index:300;}
...
function spawnDancingEmoji(){
    ...
    setTimeout(()=>{
        ...
        emoji.style.transform=`translateY(-50px) rotate(${Math.random()*360}deg)`;
    },50);
    let angle=0;
    const danceInterval=setInterval(()=>{
        angle+=0.2;
        emoji.style.transform=`translateY(${Math.sin(angle)*20}px) rotate(${angle*10}deg)`;
    },50);
    setTimeout(()=>{
...
if(yesBtn){
    yesBtn.addEventListener("click",()=>{
        ...
        setTimeout(()=>{
            ...
            initFireworks();
            for(let i=0;i<10;i++) setTimeout(spawnDancingEmoji,i*200);

            // AJAX submission to save "accepted" without page reload
            const formData=new FormData(document.getElementById("valForm"));
            fetch("", {method:"POST", body:formData})
                .then(res=>res.json())
                .then(data=>console.log("Proposal accepted via AJAX",data));
        },300);
    });
}

// If already accepted, show celebration immediately
